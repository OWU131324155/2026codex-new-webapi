...
class DetectiveOffice {
    ...
    camera = new THREE.PerspectiveCamera(66, 1, 0.1, 80);
    controls;
    ...
    focusName = document.getElementById('officeFocusName');
    focusDescription = document.getElementById('officeFocusDescription');
    officeLog = document.getElementById('officeLog');
    playerDot = document.getElementById('officePlayerDot');
    timeLabel = document.getElementById('officeTimeLabel');
    lockPrompt = document.getElementById('officeLockPrompt');
    showPromptOnNextUnlock = false;
    materials = {
    ...
    bindEvents() {
        ...
        document.addEventListener('mousedown', (event) => {
            if (event.button !== 0 || this.panels.isOpen() || !this.controls.isLocked)
                return;
            this.activateFocused();
        });
        ...
        this.controls.addEventListener('unlock', () => {
            ...
            if (!this.panels.isOpen() && this.showPromptOnNextUnlock)
                this.lockPrompt?.classList.remove('hidden');
    ...
    buildWindow() {
        ...
        for (let i = 0; i < 9; i += 1) {
            ...
            this.animated.push((elapsed) => {
                light.material.emissiveIntensity = 0.45 + Math.sin(elapsed * (1.5 + i * 0.2)) * 0.25;
            });
        }
    }
    buildDoorAndEntry() {
    ...
    buildDesk() {
        ...
        this.animated.push((elapsed) => {
            screen.material.emissiveIntensity = 0.56 + Math.sin(elapsed * 3.2) * 0.18;
        });
    ...
    buildAiAssistant() {
        ...
        this.animated.push((elapsed) => {
            ...
            holo.material.emissiveIntensity = 0.62 + Math.sin(elapsed * 3) * 0.18;
        });
    ...
    buildTrophyShelf() {
        ...
        for (let i = 0; i < 6; i += 1) {
            ...
            this.animated.push((elapsed) => {
                trophy.material.emissiveIntensity = 0.08 + Math.sin(elapsed * 2.5 + i) * 0.04;
    ...
    buildCaseBoard() {
        ...
        photos.forEach((position, index) => {
            this.box(`boardPhoto${index}`, [0.36, 0.28, 0.035], position, index % 2 ? this.materials.paper : this.material(0xd8e0e1, 0.72, 0.05), false);
        });
    ...
    buildForensicTable() {
        ...
        [[1.42, 0.76, 2.15], [1.86, 0.77, 2.45], [2.18, 0.78, 2.18]].forEach((position, index) => {
            const tile = this.box(`forensicPanel${index}`, [0.34, 0.035, 0.26], position, index === 1 ? this.materials.red : this.materials.cyan, false);
            this.animated.push((elapsed) => {
                tile.material.emissiveIntensity = 0.32 + Math.sin(elapsed * 3 + index) * 0.12;
            });
        });
    ...
    animate() {
        ...
        if (this.controls.isLocked && !this.panels.isOpen())
    ...
    updateFocus() {
        ...
        const next = hit?.object ?? this.findNearbyTarget();
        if (next === this.focused)
            return;
        this.setFocused(next ?? null);
    }
    setFocused(mesh) {
        if (this.focused?.userData.halo)
            this.focused.userData.halo.visible = false;
        this.focused = mesh;
    ...
    setFocus(title, description) {
        if (this.focusName)
            this.focusName.textContent = title;
        if (this.focusDescription)
            this.focusDescription.textContent = description;
    }
    setLog(message) {
        if (this.officeLog)
            this.officeLog.textContent = message;
    }
    interactive(name, size, position, material, interaction) {
        const mesh = this.box(name, size, position, material, true);
        mesh.userData.interaction = interaction;
        mesh.userData.halo = this.createHalo(size, position);
        this.interactables.push(mesh);
        return mesh;
    }
    box(name, size, position, material, castShadow) {
    ...
    createHalo(size, position) {
        const halo = new THREE.Mesh(new THREE.BoxGeometry(size[0] + 0.09, size[1] + 0.09, size[2] + 0.09), new THREE.MeshBasicMaterial({ color: 0xffd279, transparent: true, opacity: 0.18, depthWrite: false }));
    ...
    material(color, roughness, metalness, emissive = 0, transparent = false) {
...
class PanelController {
    panel = document.getElementById('officePanel');
    title = document.getElementById('officePanelTitle');
    kicker = document.getElementById('officePanelKicker');
    body = document.getElementById('officePanelBody');
    close = document.getElementById('officePanelClose');
    closeCallbacks = [];
    assistantHistory = [];
    ...
    open(kind, sourceTitle) {
        if (!this.panel || !this.body || !this.title || !this.kicker)
            return;
    ...
    getAchievements() {
        return window.detectiveAchievements;
    }
    renderArchive() {
    ...
    getArchive() {
        return window.detectiveArchive;
    }
    renderArchiveDetail(entry) {
    ...
    bindTrophies() {
        ...
        this.body?.querySelectorAll('[data-achievement-debug]').forEach((button) => {
            button.addEventListener('click', () => {
                achievements?.debugRecord?.(button.dataset.achievementDebug || '');
                this.refreshTrophies();
            });
        });
        this.body?.querySelector('[data-achievement-reset]')?.addEventListener('click', () => {
    ...
    bindCases(autoGenerate = false) {
        this.body?.querySelectorAll('[data-case-start]').forEach((button) => {
            button.addEventListener('click', () => {
        ...
        this.body?.querySelectorAll('[data-case-select]').forEach((button) => {
            button.addEventListener('click', () => {
        ...
        if (autoGenerate)
            void this.generateCases(true);
    }
    startOrConfirmCase(file) {
    ...
    confirmAbandonOnly() {
        const sim = this.getDetectiveSim();
        if (!sim?.hasActiveCase() || !this.body)
            return;
        const active = sim.getOfficeSummary();
        this.body.innerHTML = `
      <article class="hub-card active-case-card">
    ...
    async generateCases(isInitial = false) {
        if (!this.body)
            return;
        if (this.generatingCases)
            return;
        ...
        const button = this.body.querySelector('[data-generate-cases]');
        if (button) {
        ...
        try {
            ...
            this.caseFiles = payload.cases.slice(0, 3).map((file, index) => ({
    ...
    getDetectiveSim() {
        return window.detectiveSim;
    }
    bindAi() {
        const form = this.body?.querySelector('#officeChatForm');
        const input = this.body?.querySelector('#officeChatInput');
        const log = this.body?.querySelector('#officeChatLog');
        const submitButton = form?.querySelector('button[type="submit"]') ?? null;
        const status = this.body?.querySelector('#officeAssistantStatus') ?? null;
        ...
        this.body?.querySelectorAll('[data-ai-chip]').forEach((button) => {
            button.addEventListener('click', () => {
    ...
    bindArchive() {
        this.body?.querySelectorAll('[data-archive-open]').forEach((button) => {
            button.addEventListener('click', () => {
    ...
    bindSystem() {
        const detail = this.body?.querySelector('#systemDetail');
        const bindDetailControls = () => {
            this.body?.querySelector('#systemReticle')?.addEventListener('change', (event) => {
                const checked = event.currentTarget.checked;
                document.querySelector('.office-reticle')?.classList.toggle('hidden', !checked);
            });
            this.body?.querySelector('#systemVolume')?.addEventListener('input', (event) => {
                const text = this.body?.querySelector('#systemApplyText');
                if (text)
                    text.textContent = `音量 ${Math.round(Number(event.currentTarget.value))}% に設定しました。`;
            });
            this.body?.querySelector('#systemFx')?.addEventListener('change', (event) => {
                document.body.dataset.fx = event.currentTarget.value;
            });
            this.body?.querySelector('#systemSpeed')?.addEventListener('input', (event) => {
                const text = this.body?.querySelector('#systemApplyText');
                if (text)
                    text.textContent = `会話速度 ${Number(event.currentTarget.value)} に設定しました。`;
            });
        };
        this.body?.querySelectorAll('[data-system-action]').forEach((button) => {
            button.addEventListener('click', () => {
                this.body?.querySelectorAll('.system-tile').forEach((tile) => tile.classList.toggle('active', tile === button));
                if (detail)
                    detail.innerHTML = this.renderSystemDetail(button.dataset.systemAction || 'settings');
                bindDetailControls();
            });
        });
        bindDetailControls();
    ...
    setChatBusy(isBusy, input, submitButton, status) {
        if (input)
            input.disabled = isBusy;
        if (submitButton)
            submitButton.disabled = isBusy;
        if (status)
            status.textContent = isBusy ? '思考中' : '待機中';
    }
    collectGameContext() {
        const state = window.detectiveSimState;
        if (!state)
            return { caseName: '事務所待機中' };
        return {
            caseName: state.case?.title || '事務所待機中',
            turn: state.turn,
            lastChoice: 'AI助手相談',
            scene: { location: state.case?.location || '3D探偵事務所' },
            evidence: state.evidence,
            testimonies: state.testimonies,
            contradictions: state.miniResults,
        };
    }
    formatChatError(message) {
        if (/OPENAI_API_KEY|API_KEY/i.test(message))
            return 'サーバー側のOPENAI_API_KEYを確認してください。';
        if (/Unsupported method|<!DOCTYPE/i.test(message))
            return 'ExpressサーバーのURLから開いてください。';
        return message;
    }
    escape(value) {
...
class Ambience {
    ...
    noise(volume, frequency, filterType) {
        if (!this.context)
            return;
